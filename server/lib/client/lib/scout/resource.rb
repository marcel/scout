module Scout
  class Resource
    ...
    def uri
      ...
      sub_resource_set? ? [base, sub_resource.uri].join('/') : base
    end
    ...
    def resources
      sub_resource_set? ? [self, sub_resource.resources].flatten.compact : [self]
    end
    
    def sub_resource_set?
      !@sub_resource.nil?
    end
    ...
    private
      # N.B. Ruby 1.9 work around for implicit to_ary invocation on flatten causing infinite loop in
      # method_missing
      def to_ary
        nil
      end
